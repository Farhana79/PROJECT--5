...
template <class T>
void DoublyLinkedList<T>::swap(size_t position1, size_t position2)
{
    if (position1 == position2) // nothing to swap
        return;

    // get pointers to the nodes at the specified positions
    Node<T>* node1_ptr = getPointerTo(position1);
    Node<T>* node2_ptr = getPointerTo(position2);

    if (node1_ptr == nullptr || node2_ptr == nullptr) // one or both positions are invalid
        return;

    // swap the nodes
    Node<T>* temp_prev = node1_ptr->getPrevious();
    Node<T>* temp_next = node1_ptr->getNext();
    node1_ptr->setPrevious(node2_ptr->getPrevious());
    node1_ptr->setNext(node2_ptr->getNext());
    node2_ptr->setPrevious(temp_prev);
    node2_ptr->setNext(temp_next);

    // update the adjacent nodes' links to point to the new nodes
    if (node1_ptr->getPrevious() == nullptr) // node1 was the first node
        first_ = node2_ptr;
    else
        node1_ptr->getPrevious()->setNext(node2_ptr);

    if (node1_ptr->getNext() == nullptr) // node1 was the last node
        last_ = node2_ptr;
    else
        node1_ptr->getNext()->setPrevious(node2_ptr);

    if (node2_ptr->getPrevious() == nullptr) // node2 was the first node
        first_ = node1_ptr;
    else
        node2_ptr->getPrevious()->setNext(node1_ptr);

    if (node2_ptr->getNext() == nullptr) // node2 was the last node
        last_ = node1_ptr;
    else
        node2_ptr->getNext()->setPrevious(node1_ptr);
}
